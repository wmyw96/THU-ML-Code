...
import random
import sys
from estimator.rkhs import RKHSEstimator
...
def pmf(observed, n, m, D, n_particles, select_uid, select_vid, alpha_u,
    ...
    with zs.BayesianNet(observed=observed) as model:
        ...
        print('c', tf.convert_to_tensor(z).get_shape())
        select_u = select_from_axis1(z, select_uid)
        print('b', tf.convert_to_tensor(v).get_shape())
...
def c_matmul(x, y, K, left_need_expand):
    if left_need_expand:
        x = tf.tile(tf.expand_dims(x, 0), [K, 1, 1])
    else:
...
def MMNN(K, m, n):
    n_ev = 200
    cur_r, cur_c = 30, 30
    mu_v = tf.zeros(shape=[cur_r, cur_c])
    log_std_v = tf.zeros(shape=[cur_r, cur_c])
    av = zs.Normal('av', mu_v, logstd=log_std_v,
                   n_samples=K, group_event_ndims=1)  # [K, 10, 10]
    aev = zs.Normal('aev', mu_v, logstd=log_std_v,
                    n_samples=n_ev, group_event_ndims=1)  # [n_ev, 10, 10]
    layer = [(m, n)]
    L = len(layer)
    for i in range(L):
        nxt_r, nxt_c = layer[i]
        Wl = tf.get_variable('Wl_%d' % i, shape=[nxt_r, cur_r],
                             initializer=tf.random_normal_initializer(0, 0.1))
        Bl = tf.get_variable('Bl_%d' % i, shape=[nxt_r, cur_c],
                             initializer=tf.random_normal_initializer(0, 0.1))
        av = c_matmul(Wl, av, K, True) + Bl
        aev = c_matmul(Wl, aev, n_ev, True) + Bl
        Wr = tf.get_variable('Wr_%d' % i, shape=[cur_c, nxt_c],
                             initializer=tf.random_normal_initializer(0, 0.1))
        Br = tf.get_variable('Br_%d' % i, shape=[nxt_r, nxt_c],
                             initializer=tf.random_normal_initializer(0, 0.1))
        av = c_matmul(av, Wr, K, False) + Br
        aev = c_matmul(aev, Wr, n_ev, False) + Br
        cur_r, cur_c = nxt_r, nxt_c
        if i < L - 1:
            av = tf.nn.relu(av)
            aev = tf.nn.relu(aev)
    est = RKHSEstimator()
    qx_samples = tf.reshape(aev, [n_ev, n * m])
    px_samples = tf.random_normal(shape=(n_ev, m * n), mean=0, stddev=1, dtype=tf.float32)
    r = est.optimal_ratio(tf.reshape(av, [K, m * n]), qx_samples, px_samples)
    return av, tf.log(r)
...
def q_net(observed, ratings, indices, portion, n, D, m, n_particles,
    ...
    with zs.BayesianNet(observed=observed) as variational:
        ...
        z = zs.Normal('z', z_mean, logstd=z_log_std, n_samples=None,
                      group_event_ndims=1)
        print(tf.convert_to_tensor(z).get_shape())
    ...
    return variational, v, kl_p, kl_q
