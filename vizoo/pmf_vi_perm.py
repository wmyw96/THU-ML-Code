...
def get_test_data_combined(M, head, tail, user_movie, user_movie_score,
                           user_movie_test, user_movie_score_test, fake_pred):
    i_indices = []
    i_ratings = []
    i_coeff = []
    uid = []
    ...
    for i in range(tail - head):
        ...
        for j in range(len(user_movie[user_id])):
            i_ratings += [user_movie_score[user_id][j] - bias]
        for j in range(len(user_movie_test[user_id])):
            i_ratings += [fake_pred[tot] * np.sqrt(2)]
            tot += 1
        i_coeff += [len(user_movie_score[user_id]) + len(user_movie_test[user_id])]
        i_indices += user_movie[user_id]
        i_indices += user_movie_test[user_id]
    ...
    for i in range(len(i_coeff)):
        for j in range(i_coeff[i]):
            i_portion[i][id] = 1.0 / max(i_coeff[i], 1)
            id += 1
    return i_indices, i_ratings, i_portion
                ...
                for t in range(valid_iters):
                    ...
                    in_ins, in_rts, in_port, out_uid, out_vid, out_rating = \
                        get_test_data(M, l, r, user_movie, user_movie_score,
                                      user_movie_valid, user_movie_score_valid)
                    for j in range(2):
                        #in_ins, in_rts, in_port = \
                        #    get_test_data_combined(M, l, r, user_movie, user_movie_score,
                        #        user_movie_valid, user_movie_score_valid, out_rating)
                        _, pd = sess.run([se, pred],
                                      feed_dict={n: cur_batch_size,
                                                 m: M,
                                                 is_training: False,
                                                 n_particles: K * 100,
