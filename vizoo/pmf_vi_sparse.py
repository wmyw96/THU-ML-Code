...
def q_net(observed, ratings, indices, portion, n, D, m, n_particles,
    ...
    with zs.BayesianNet(observed=observed) as variational:
        ...
        n_hidden = int(lh_r.get_shape()[-1])
        matrix_lh_r = tf.reshape(lh_r, [-1, n_hidden])
        hd = tf.sparse_tensor_dense_matmul(portion, matrix_lh_r)
        hd = tf.reshape(hd, [n_particles, 
            tf.div(tf.shape(hd)[-2], n_particles), n_hidden])
        print(tf.shape(hd))
        ...
        for i in range(len(i_coeff)):
            ...
            for j in range(i_coeff[i]):
                if np.random.uniform(0, 1.0) < keep_prob:
                    portion_indices.append([k * bsize + i, k * msize + id])
                    cnt += 1
                id += 1
            tot_cnt += cnt
            for j in range(cnt):
                portion_value.append(1.0 / cnt)
    if tot_cnt == 0:
        portion_indices.append([])
        cc = 0
    i_portion = tf.SparseTensorValue(np.array(portion_indices),
                                     np.array(portion_value),
                                     [n_particles * len(i_coeff), n_particles * sum(i_coeff)])
    return i_indices, i_ratings, i_portion, uid, vid, i_ratings, cc


def get_test_data(M, head, tail, user_movie, user_movie_score,
                  user_movie_test, user_movie_score_test, n_particles):
    i_indices = []
    i_ratings = []
    i_coeff = []
    uid = []
    ...
    for k in range(n_particles):
        ...
        for i in range(len(i_coeff)):
            for j in range(i_coeff[i]):
                portion_indices.append([k * bsize + i, k * msize + id])
                portion_value.append(1.0 / i_coeff[i])
                id += 1
    i_portion = tf.SparseTensorValue(np.array(portion_indices),
                                     np.array(portion_value),
                                     [n_particles * len(i_coeff), n_particles * sum(i_coeff)])
...
if __name__ == '__main__':
    ...
    infer_portion = tf.sparse_placeholder(tf.float32, shape=[None, None],
        name='infer_portion')
        ...
        for epoch in range(num_epochs):
            ...
            for t in range(iters):
                ...
                tr_ins, tr_rts, tr_port, tr_u, tr_v, tr_rating, cc = \
                    get_traing_data(M, l, r, idxes, user_movie,
                                    user_movie_score, K, constant_kp)
                if cc == 0:
                    continue
            ...
            if (epoch + 1) % 10 == 0:
                ...
                for t in range(valid_iters):
                    ...
                    in_ins, in_rts, in_port, out_uid, out_vid, out_rating = \
                        get_test_data(M, l, r, user_movie, user_movie_score,
                                      user_movie_valid, user_movie_score_valid, K)
                    __ = sess.run(se,
                                  feed_dict={n: cur_batch_size,
                                             m: M,
                                             is_training: False,
                ...
                for t in range(test_iters):
                    ...
                    in_ins, in_rts, in_port, out_uid, out_vid, out_rating = \
                        get_test_data(M, l, r, user_movie, user_movie_score,
                                      user_movie_test, user_movie_score_test, K)
                    __ = sess.run(se,
                                  feed_dict={n: cur_batch_size,
                                             m: M,
                                             is_training: False,
