...
@zs.reuse('model')
def pmf(observed, n, m, D, n_particles, select_u, select_v, hdim,
    ...
    with zs.BayesianNet(observed=observed) as model:
        ...
        collection = tf.concat([tf.expand_dims(gather_u, 0),
                                tf.expand_dims(gather_v, 0)], axis=0)
        lh_c = layers.fully_connected(collection, 300,
                                      activation_fn=tf.nn.relu)
        h = layers.fully_connected(lh_c, hdim, activation_fn=None)
        pred_mu = tf.reduce_sum(tf.reduce_prod(h, axis=0), axis=2)
...
if __name__ == '__main__':
    ...
    def log_joint(observed):
        model, _ = pmf(observed, N, M, D, n_particles, select_u,
                       select_v, D2, alpha_u, alpha_v, alpha_pred)
    ...
    _, pred_rating = pmf({'u': qu_samples, 'v': qv_samples,(
                         ...
                         select_u, select_v, D2, alpha_u, alpha_v, alpha_pred)
