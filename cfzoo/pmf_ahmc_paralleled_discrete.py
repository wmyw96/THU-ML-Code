...
def pmf(observed, n, m, D, n_particles, select_uid, select_vid,
    ...
    with zs.BayesianNet(observed=observed) as model:
        ...
        mu_v = tf.zeros(shape=[m, 5, D])
        log_std_v = tf.ones(shape=[m, 5, D]) * tf.log(alpha_v)
        v = zs.Normal('v', mu_v, log_std_v,
                      n_samples=n_particles, group_event_ndims=1)
        select_u = select_from_axis(u, select_uid, 1, 3)   # [K, B, D]
        select_v = select_from_axis(v, select_vid, 1, 4)   # [K, B, nR, D]
        select_u = tf.tile(tf.expand_dims(select_u, 2), [1, 1, 5, 1])
...
if __name__ == '__main__':
    ...
    result_path = 'tmp/pmf_ahmc_paralleled/'
    ...
    for i in range(M / chunk_size):
        vi = tf.get_variable('v_chunk_%d' % i, shape=[K, chunk_size, 5, D],
                             initializer=tf.random_normal_initializer(0, 0.1),
                             trainable=False)
        Vs.append(vi)
    ...
    true_rating_2d = tf.one_hot(true_rating_2d - 1, depth=5)
    true_rating_2d = tf.cast(true_rating_2d, tf.int32)
    ...
    hmc_v = zs.HMC(step_size=1e-3, n_leapfrogs=10, adapt_step_size=None,
                   target_acceptance_rate=0.9)
    target_u = select_from_axis(U, select_u, 1, 3)
    target_v = select_from_axis(V, select_v, 1, 4)
    ...
    candidate_sample_u = \
        tf.get_variable('cand_sample_chunk_u', shape=[K, chunk_size, D],
                        ...
                        trainable=True)
    candidate_sample_v = \
        tf.get_variable('cand_sample_chunk_v', shape=[K, chunk_size, 5, D],
                        initializer=tf.random_normal_initializer(0, 0.1),
                        trainable=True)
    ...
    candidate_idx_v = tf.placeholder(tf.int32, shape=[chunk_size, ],
                                     name='cand_v_chunk')
    candidate_u = select_from_axis(U, candidate_idx_u, 1, 3)  # [K, chunk_size, D]
    candidate_v = select_from_axis(V, candidate_idx_v, 1, 4)  # [K, chunk_size, 5, D]
