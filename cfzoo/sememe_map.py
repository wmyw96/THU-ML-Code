...
if __name__ == '__main__':
    ...
    U_left = tf.Variable(random_weights(N, D))     # (N, D)
    U_right = tf.Variable(random_weights(D, D2))
    U_right /= tf.sqrt(tf.reduce_sum(U_right * U_right, axis=1, keep_dims=True))
    U = tf.matmul(U_left, U_right)
    V_left = tf.Variable(random_weights(M, D))     # (M, D)
    V_right = tf.Variable(random_weights(D, D2))
    V_right /= tf.sqrt(tf.reduce_sum(V_right * V_right, axis=1, keep_dims=True))
    V = tf.matmul(V_left, V_right)
    ...
    cost = 0.5 * tf.reduce_sum(error * error) * \
        ...
        lambda_U * 0.5 * tf.reduce_sum(U_left * U_left, axis=[0, 1]) + \
